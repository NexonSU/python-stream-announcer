...
if not os.path.exists(f'{temp_folder_name}/categorylist'):
	categorylist = requests.get('https://www.googleapis.com/youtube/v3/videoCategories', params={'part': 'snippet', 'regionCode': 'ru', 'hl': 'ru_RU', 'key': youtube_api_key}).text
	open(f'{temp_folder_name}/categorylist', 'wt', encoding='utf-8').write(categorylist)

#checking for videoid on stream page 
stream = requests.get('https://www.youtube.com/embed/live_stream', params={'channel': youtube_channelid}).content
soup = BeautifulSoup(stream, features="html.parser")
videoid = soup.head.find_all('link', rel="canonical")[0]['href'].split('watch?v=')[1]

#getting stream information
stream = requests.get('https://www.googleapis.com/youtube/v3/videos', params={'part': 'snippet,liveStreamingDetails', 'id': videoid, 'key': youtube_api_key}).json().get("items")[0]
